...
# Try to import from openai-agents package with fallbacks for missing functions
try:
    from agents import Agent, OpenAIChatCompletionsModel, AsyncOpenAI, function_tool, set_tracing_disabled, enable_verbose_stdout_logging
    import agents
...
Provide comprehensive, detailed answers based on the retrieved passages.
When quoting, provide sufficient context for understanding, with quotes up to 1000 characters when needed for completeness.
""",
        model=model,
        tools=[retrieve_tool]
    )
